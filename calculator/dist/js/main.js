...
that.xPct.push(percentage);that.xVal.push(value[0]);if(!percentage){if(!isNaN(value[1])){that.xSteps[0]=value[1];}}
...
if(ordered.length&&typeof ordered[0][0]==="object"){ordered.sort(function(a,b){return a[0][0]-b[0][0];});}
else{ordered.sort(function(a,b){return a[0]-b[0];});}
...
if(entry===true){parsed.tooltips=[];for(var i=0;i<parsed.handles;i++){parsed.tooltips.push(true);}}
...
if(handleNumbers.length>1){handleNumbers.forEach(function(handleNumber,o){var to=checkHandlePosition(proposals,handleNumber,proposals[handleNumber]+proposal,b[o],f[o],false);if(to===false){proposal=0;}
...
return{__spectrum:Spectrum,version:VERSION,cssClasses:cssClasses,create:initialize};});(function(){'use strict';document.addEventListener('DOMContentLoaded',()=>{const inputs=document.querySelectorAll('.js-input');const popupCloseTriggers=document.querySelectorAll('.js-popup-close');const popupOpenTriggers=document.querySelectorAll('.js-popup-trigger');const hiddenInput=document.querySelector('.js-hidden-input');const closePopups=()=>{const popups=document.querySelectorAll('.js-popup');popups.forEach(function(popup){popup.classList.remove('active');});toggleScroll(true);};const openPopup=element=>{closePopups();let units=document.querySelectorAll('.js-part');units=[...units];const unitsData=units.map(item=>{return{name:item.querySelector('.part__name').innerText,price:item.querySelector('.part__price').innerText,}});hiddenInput.value=JSON.stringify(unitsData);const targetPopup=typeof element=='string'?document.querySelector(element):element;targetPopup.classList.add('active');toggleScroll(false);return targetPopup;};const openPopupByTrigger=()=>{let target=event.target;while(!target.classList.contains('js-popup-trigger')){target=target.parentNode;}
openPopup(target.dataset.target);};const getScrollWidth=()=>{const div=document.createElement('div');div.style.overflowY='scroll';div.style.width='50px';div.style.height='50px';document.body.append(div);const scrollWidth=div.offsetWidth-div.clientWidth;div.remove();return scrollWidth;};const toggleScroll=(state=null)=>{const body=document.querySelector('body');const html=document.querySelector('html');const scrollWidth=getScrollWidth();if(state!=null){state?body.classList.remove('no-scroll'):body.classList.add('no-scroll');state?html.classList.remove('no-scroll'):html.classList.add('no-scroll');body.style.marginRight=state?`${0}px`:`${scrollWidth}px`;}
else{body.classList.toggle('no-scroll');html.classList.toggle('no-scroll');}};const getRangePrice=range=>{const coefficient=Number(range.dataset.coefficient);const value=parseFloat(range.noUiSlider.get());return Math.ceil(coefficient*value);};const createSliders=()=>{inputs.forEach(item=>{const range=item.querySelector('.js-range');const pipsContainer=item.querySelector('.js-range-pips');const minValue=Number(range.dataset.rangeMin);const maxValue=Number(range.dataset.rangeMax);const createStep=text=>{const step=document.createElement('span');step.classList.add('input__step');step.innerHTML=text;return step;};noUiSlider.create(range,{start:[Number(range.dataset.start)],connect:'lower',range:{'min':[minValue],'max':[maxValue+minValue],},});for(let i=minValue;i<=maxValue;i+=minValue){let pip=createStep(i);pipsContainer.appendChild(pip);if(i==maxValue){let pip=createStep(i);pipsContainer.appendChild(pip);}}
range.noUiSlider.on('update',()=>{const value=parseFloat(range.noUiSlider.get());if(value==maxValue+minValue){openPopup('.js-popup-personal');}});});};const fillDiagram=()=>{const colorArr=['#ffd500','#ff7600','#a66f00','#ffe873','#ffb005'];const vat=document.querySelector('.js-vat');const createEl=(tagNmae,classList)=>{const item=document.createElement(tagNmae);classList.forEach(className=>item.classList.add(className));return item;};const setItemPrice=(item,price)=>{item.querySelector('.part__price').innerHTML=price+'€';};const calcSumm=inputs=>{let summ=[...inputs].reduce((total,curr)=>{curr=getRangePrice(curr.querySelector('.js-range'));total=typeof total!='number'?getRangePrice(total.querySelector('.js-range')):total;return total+curr;});if(vat.checked)summ*=1.5;return Math.ceil(summ);};const createLegendItem=(name,price,color)=>{const item=createEl('li',['diagram__item','part','js-part']);const itemColor=createEl('span',['part__color']);itemColor.setAttribute('style',`background-color: ${color}`);const itemName=createEl('span',['part__name']);itemName.innerHTML=name.replace(' в месяц:','').replace(':','');const itemPrice=createEl('span',['part__price']);itemPrice.innerHTML=price+'€';item.appendChild(itemColor);item.appendChild(itemName);item.appendChild(itemPrice);return item;};const setCircleUnit=(index,prevPercent,percent,color)=>{const circle=document.querySelector('.js-circle').querySelectorAll('circle')[index];circle.setAttribute('r','15.9');circle.setAttribute('cx','50%');circle.setAttribute('cy','50%');circle.setAttribute('style',`stroke: ${color}; stroke-dasharray: ${percent} 100; stroke-dashoffset: ${-prevPercent};`);return circle;};const getPrevPercent=index=>{if(index<=0)return 0;const range=inputs[index-1].querySelector('.js-range');const price=getRangePrice(range);const percent=(price/calcSumm(inputs))*100;return percent+getPrevPercent(index-1);};const setEventListeners=(legendItem,circleUnit)=>{legendItem.addEventListener('mouseover',()=>{circleUnit.classList.add('hovered');});legendItem.addEventListener('mouseleave',()=>{circleUnit.classList.remove('hovered');});circleUnit.addEventListener('mouseover',()=>{legendItem.classList.add('hovered');});circleUnit.addEventListener('mouseleave',()=>{legendItem.classList.remove('hovered');});};const checkVat=()=>{const vatUnit=document.querySelector('.js-vat-unit');const legend=document.querySelector('.js-diagram-legend');vatUnit.setAttribute('style',`stroke-dasharray: 0 100; stroke: ${colorArr[colorArr.length - 1]}`);document.querySelector('.js-circle-price').innerHTML=calcSumm(inputs)+'€';try{document.querySelector('.js-vat-item').remove();}
catch{}
const legendItem=createLegendItem(vat.previousElementSibling.innerHTML,Math.floor(calcSumm(inputs)*(1-1/1.5)),colorArr[colorArr.length-1]);if(vat.checked){let percent=Math.floor(calcSumm(inputs)*(1-1/1.5))/calcSumm(inputs)*100;vatUnit.setAttribute('style',`stroke-dasharray: ${percent} 100; stroke: ${colorArr[colorArr.length - 1]}; stroke-dashoffset: ${100 - percent}`);legendItem.classList.add('js-vat-item');legend.appendChild(legendItem);}
inputs.forEach((item,index)=>{const range=item.querySelector('.js-range');const price=getRangePrice(range);const percent=(price/calcSumm(inputs))*100;const prevPercent=getPrevPercent(index);setCircleUnit(index,prevPercent,percent,colorArr[index]);});setEventListeners(legendItem,vatUnit);};inputs.forEach((item,index)=>{const range=item.querySelector('.js-range');const legend=document.querySelector('.js-diagram-legend');const price=getRangePrice(range);const label=item.querySelector('.js-input-label').innerHTML;document.querySelector('.js-circle-price').innerHTML=calcSumm(inputs)+'€';const percent=(price/calcSumm(inputs))*100;const prevPercent=getPrevPercent(index);const legendItem=createLegendItem(label,price,colorArr[index]);const circleUnit=setCircleUnit(index,prevPercent,percent,colorArr[index]);setEventListeners(legendItem,circleUnit);range.noUiSlider.on('update',()=>{const price=getRangePrice(range);setItemPrice(legendItem,price);document.querySelector('.js-circle-price').innerHTML=calcSumm(inputs)+'€';inputs.forEach((item,index)=>{const range=item.querySelector('.js-range');const price=getRangePrice(range);const percent=(price/calcSumm(inputs))*100;const prevPercent=getPrevPercent(index);setCircleUnit(index,prevPercent,percent,colorArr[index]);});checkVat();});legend.appendChild(legendItem);});vat.addEventListener('change',checkVat);checkVat();};popupCloseTriggers.forEach(trigger=>trigger.addEventListener('click',()=>{event.stopPropagation();if(event.target.classList.contains('js-popup-close')){closePopups();}}));popupOpenTriggers.forEach(trigger=>trigger.addEventListener('click',openPopupByTrigger));createSliders();fillDiagram();});}());