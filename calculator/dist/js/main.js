...
return{__spectrum:Spectrum,version:VERSION,cssClasses:cssClasses,create:initialize};});(function(){'use strict';document.addEventListener('DOMContentLoaded',()=>{const inputs=document.querySelectorAll('.js-input');const popupCloseTriggers=document.querySelectorAll('.js-popup-close');const closePopups=()=>{const popups=document.querySelectorAll('.js-popup');popups.forEach(function(popup){popup.classList.remove('active');});toggleScroll(true);};const openPopup=element=>{closePopups();const targetPopup=typeof element=='string'?document.querySelector(element):element;targetPopup.classList.add('active');toggleScroll(false);return targetPopup;};const getScrollWidth=()=>{const div=document.createElement('div');div.style.overflowY='scroll';div.style.width='50px';div.style.height='50px';document.body.append(div);const scrollWidth=div.offsetWidth-div.clientWidth;div.remove();return scrollWidth;};const toggleScroll=(state=null)=>{const body=document.querySelector('body');const html=document.querySelector('html');const scrollWidth=getScrollWidth();if(state!=null){state?body.classList.remove('no-scroll'):body.classList.add('no-scroll');state?html.classList.remove('no-scroll'):html.classList.add('no-scroll');body.style.marginRight=state?`${0}px`:`${scrollWidth}px`;}
