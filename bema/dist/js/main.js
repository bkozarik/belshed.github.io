...
document.addEventListener('DOMContentLoaded',function(){const header=document.querySelector('.header');const burger=document.querySelector('.js-burger');const menu=document.querySelector('.js-menu');const tabTriggers=document.querySelectorAll('.js-tab-trigger');const tabs=document.querySelectorAll('.js-tab');const mouseMoveElements=document.querySelectorAll('.js-mousemove-item');const galleryItems=document.querySelectorAll('.js-gallery-item');const popupCloseTriggers=document.querySelectorAll('.js-popup-close');const popupTriggers=document.querySelectorAll('.js-popup-trigger');const popups=document.querySelectorAll('.popup');const passwordTriggers=document.querySelectorAll('.js-pass-toggle');function scrollHandler(){window.pageYOffset>60?header.classList.add('fixed'):header.classList.remove('fixed');}
...
function openPopup(selector){const targetPopup=document.querySelector(selector);targetPopup.classList.add('active');toggleScroll(false);return targetPopup;}
...
function toggleScroll(state){var state=state||null;const body=document.querySelector('body');const html=document.querySelector('html');if(state!=null){state?body.classList.remove('no-scroll'):body.classList.add('no-scroll');state?html.classList.remove('no-scroll'):html.classList.add('no-scroll');}
else{body.classList.toggle('no-scroll');html.classList.toggle('no-scroll');}}
function toggleMenu(state){var state=state||null;if(state==null){return function(){burger.classList.toggle('active');menu.classList.toggle('active');toggleScroll();}}
...
if(isExists('.js-gallery-swiper')){try{const gallerySwiper=new Swiper('.js-gallery-swiper',{slidesPerView:1,spaceBetween:30,speed:700,centeredSlides:true,navigation:{prevEl:'.gallery__control_prev',nextEl:'.gallery__control_next',},});}
catch(e){console.log('Свайпер не определен!!!');}}
...
if(isExists(passwordTriggers)){passwordTriggers.forEach(function(trigger){trigger.addEventListener('click',function(){this.classList.toggle('active');const type=this.classList.contains('active')?'text':'password';this.parentNode.querySelector('input').setAttribute('type',type);});});}
