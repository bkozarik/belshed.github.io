...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const qizSwiperNode = document.querySelector('.js-swiper-quiz-page');
        const qizDatesSwiperNode = document.querySelector('.js-quiz-date-swiper');
        ...
        const toggleMenu = (state = null) => {
            if(state === null){
                return () => {
                    ...
                    toggleScroll(!menu.classList.contains('active'));
                }
            }

            state ? menu.classList.add('active') : menu.classList.remove('active');
            state ? menuTrigger.classList.add('active') : menuTrigger.classList.remove('active');
        ...
        class Quiz {
            constructor (element){
                ...
                this.pagination = this.el.querySelector('.js-quiz-pagination');
                this.paginationItems = this.createPagination(this.pages.length);
                this.controls = this.el.querySelectorAll('.js-quiz-slide');
                this.questions = this.el.querySelectorAll('.js-questions-item');
                this.currentPage = 0;
            ...
            init(){
                const squareItems = this.el.querySelectorAll('.js-square-item');
                const styleItems = this.el.querySelectorAll('.js-style-item');

                this.controls.forEach(control => control.addEventListener('click', () => {
                    const direction = control.dataset.page;
                ...
                squareItems.forEach((item, index, all) => {
                ...
                styleItems.forEach((item, index, all) => {
                    item.addEventListener('click', () => {
                        all.forEach(item => item.classList.remove('active'));
            ...
            updateDimentions(){
                ...
                const currentPageHeight = currentPage.getBoundingClientRect().height;
                ...
                this.track.style.height = this.currentPage != 2 ? `${currentPageHeight}px`: `${currentPageHeight}px`;
            }
            createPagination(number){
                const paginationItems = new Array();

                for(let i = 0; i < number; i++){
                    const item = document.createElement('li');
                    item.classList.add('pagination__item', 'js-quiz-pagination-item');
            ...
            toggleControls(){
                if(this.currentPage == this.pages.length - 1){
                    document.querySelector('.quiz__question-button').classList.add('visible');

                    this.controls.forEach(control => {
                        control.classList.remove('visible');

                        if(control.classList.contains('js-quiz-slide-begin')){
                            control.classList.add('visible');
                        }
                    });
                }
                else{
                    document.querySelector('.quiz__question-button').classList.remove('visible');
                    this.controls.forEach(control => {
                        control.classList.add('visible');
            ...
            slideToPage(page) {
                ...
                this.changeQuestion(this.currentPage);
                this.toggleControls();
                this.setActivePaginationItem(this.currentPage);
        ...
        const scrollHandler = () => {
            ...
            const direction = windowScroll < currScroll ? true : false;
        ...
        const mousemoveHandler = () => {
            ...
            y > 100 ? header.classList.add('hidden') : header.classList.remove('hidden');
        };

        if(isExists(menuTrigger)){
            menuTrigger.addEventListener('click', toggleMenu());
        }
        
        if(isExists(quizEl)){
            const q = new Quiz(quizEl);
        }
