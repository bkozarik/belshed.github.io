...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const placesSwiper = document.querySelector('.js-places-swiper');
        const hotelSwiper = document.querySelector('.js-hotel-swiper');

        const faqContainer = document.querySelector('.js-faq-container');

        const dropdownItems = document.querySelectorAll('.js-dropdown');

        const quizNode = document.querySelector('.js-quiz');

        const popupOpenTriggers = document.querySelectorAll('.js-popup-trigger');
        let popupCloseTriggers = document.querySelectorAll('.js-popup-close');
        ...
        const mediaPopupTrigger = document.querySelectorAll('.js-media-popup-trigger');

        const popupSwipers = document.querySelectorAll('.js-popup-swiper-container');

        const gallery = document.querySelector('.js-gallery');

        const calendarNode = document.querySelector('.js-calendar');

        const placeItems = document.querySelectorAll('.js-place-item');
        ...
        const openPopup = element => {
            ...
            const targetPopup = typeof element == 'string' ? document.querySelector(element) : element;
        ...
        const openPopupByTrigger = () => {
            let target = event.target;

            while(!target.classList.contains('js-popup-trigger')){
        ...
        const closePopups = () => {
            ...
            popups.forEach(function(popup){
                ...
                if(isExists(popup.querySelector('video'))){
                ...
                options.forEach((option, index) => {
                    if(option.dataset.value != 'none' && !option.hasAttribute('hidden')){
                        ...
                        dropdownPopupItem.addEventListener('click', () => {
                            ...
                            let targetItemIndex = parseInt(targetItem.dataset.index);
        ...
        const setHeightToFitContent = (element, mode) => {
            ...
            const targetHeight = mode ? contentHeight : 0;
            let currentHeight = parseInt(getComputedStyle(element).height);

            let counter = 0;
            let heightAnimation = setInterval(() => {
                currentHeight = parseInt(getComputedStyle(element).height);
                element.style.height = mode ? `${counter + counterStep}px` : `${currentHeight - counter}px`;

                counter += counterStep;

                if( counter >= Math.abs(targetHeight - currentHeight)) clearInterval(heightAnimation);
            }, frameRate);
        };

        const showTab = (container, id = 0) => {
            const tabPages = container.querySelectorAll('.js-tab-content');
        ...
        const popupSliderTriggerClick = selector => {
            return () => {
                event.preventDefault();
                const target = event.target;
                const targetSlideIndex = parseInt(target.dataset.slideIndex) || 0;

                const targetPopup = openPopup(selector);

                targetPopup.querySelector('.js-popup-swiper-container').swiper.slideTo(targetSlideIndex, 0, true);
            }
        };
        ...
        class Quiz {
            constructor (element){
                ...
                this.steps = this.el.querySelectorAll('.js-quiz-step');
                this.stepsContainer = this.el.querySelector('.js-quiz-steps');
                this.track = this.el.querySelector('.js-quiz-track');
                this.pages = this.el.querySelectorAll('.js-quiz-page');
                this.progress = this.el.querySelector('.js-quiz-progress');
                this.progressBar = this.el.querySelector('.js-quiz-progress-bar');
                this.pictures = this.el.querySelector('.js-quiz-pictures');
                this.controls = this.el.querySelectorAll('.js-quiz-slide');
                this.form = this.el.querySelector('.js-quiz-form');

                this.currentPage = 0;
                this.slideToPage(this.currentPage);

                this.stepsContainer.addEventListener('click', () => {
                    event.preventDefault();

                    if(event.target.classList.contains('js-quiz-step') && !event.target.classList.contains('disabled')){
                        const targetPage = parseInt(event.target.getAttribute('href'));
                        
                        this.slideToPage(targetPage);
                    }
                });

                this.controls.forEach(control => control.addEventListener('click', () => {
                    let targetPage = control.classList.contains('js-quiz-slide-next') ? this.currentPage + 1 : control.classList.contains('js-quiz-slide-prev') ? this.currentPage - 1 : this.currentPage;
            ...
            setActiveQuizStep(step) {
                ...
                this.setQuizProgressBarWidth(step);
            }
            setActivePicture(step){
                let items = [...this.pictures.childNodes];
                items = items.filter(node => node.classList && node.classList.contains('js-quiz-pic'));
            ...
            setQuizProgressBarWidth(step) {
                const progressWidth = this.progress.getBoundingClientRect().width;

                this.progressBar.style.width = (progressWidth * (step + 1)) / this.steps.length + 'px';
            }
            setPageToActive (step) {
            ...
            toggleControls(){
                const controlsArr = [...this.controls];

                switch (this.currentPage){
                    case 0:
                        controlsArr.filter(item => item.classList.contains('js-quiz-slide-prev'))[0].classList.remove('visible');
                    break;
                    case this.pages.length - 1:
                        controlsArr.filter(item => item.classList.contains('js-quiz-slide-next'))[0].classList.remove('visible');
                        controlsArr.filter(item => item.classList.contains('js-quiz-slide-submit'))[0].classList.add('visible');
                    break;
                    default:
                        controlsArr.forEach(item => {
                            if(!item.classList.contains('js-quiz-slide-submit')) item.classList.add('visible');
            ...
            slideToPage(page) {
                this.currentPage = constrain(page, 0, this.pages.length - 1);
                this.setPageToActive(this.currentPage);
                this.toggleControls();
                this.setActiveQuizStep(this.currentPage);
                this.setActivePicture(this.currentPage);

                const pageWidth = this.track.getBoundingClientRect().width;
                this.track.style.transform = `translateX(-${pageWidth * this.currentPage}px)`;
                ...
                if(this.pages.length - 1 == page) {                
                    this.pages.forEach((page, index) => {
                        ...
                        let input = this.form.querySelector(`input[name="${namesArr[index]}"]`);
                        
                        if(isExists(page.querySelector('select'))){
                            input.setAttribute('value', page.querySelector('select').value);
                        }

                        if(isExists(page.querySelector('.js-calendar'))){
                            let dates = [];

                            page.querySelectorAll('.js-calendar-day.active').forEach(item => {
                                dates.push(item.dataset.date);
                            });

                            input.setAttribute('value', JSON.stringify(dates));
                        }

                        if(isExists(page.querySelector('.js-quiz-grid'))){
                            const addons = [...page.querySelectorAll('.js-quiz-addon input')].filter(item => item.checked);

                            const data = [];

                            addons.forEach(addon => {
                                data.push(addon.name);
                            });
        ...
        class Calendar {
            constructor (element){
                this.el = typeof element == 'string' ? document.querySelector(element) : element;
                this.body = this.el.querySelector('.js-calendar-body');
                this.dateNode = this.el.querySelector('.js-calendar-date');
                this.controls = this.el.querySelectorAll('.js-calendar-navigation');
                this.currentDate = new Date();
                this.monthsArr = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'];
                this.dates = this.el.dataset.date.split(', ');
                this.dimentions = {
                    height: 6,
                    width: 7,
                };
                this.controls.forEach(control => control.addEventListener('click', event => {
                    ...
                    while(!control.classList.contains('js-calendar-navigation')){
                    ...
                    if(control.classList.contains('js-calendar-prev-year')){
                        this.setDate(date.month, date.year - 1);
                    }
                    if(control.classList.contains('js-calendar-next-year')){
                        this.setDate(date.month, date.year + 1);
                    }
                    if(control.classList.contains('js-calendar-prev-month')){
                        this.setDate(date.month - 1, date.year);
                    }
                    if(control.classList.contains('js-calendar-next-month')){
                        this.setDate(date.month + 1, date.year);
                    }
                }));
                ...
                this.setDate(this.currentDate.getMonth(), this.currentDate.getFullYear());
            }
            transformDates(){
                this.dates = this.dates.filter(date => !!date);

                this.dates = this.dates.map(date => {
                    let dateArr = date.split('/');

                    dateArr = dateArr.map(number => /^0\d/.test(number) ? number.replace(0, '') : number);
            ...
            setDate(month, year){
                ...
                this.changeDateInNode(month, year);
                this.fillDates(month, year);
            }
            changeDateInNode(month, year){
                this.dateNode.innerHTML = `${this.monthsArr[month]} ${year}`;
            }
            fillDates(month, year){
                ...
                for (let i = 0; i < targetDate.getDay() - 1; i++) {
                    const date = document.createElement('button');
                    date.classList.add('button', 'calendar__day', 'js-calendar-day');
                    let status = 'calendar__day_disabled';

                    date.classList.add(status);
                    date.dataset.date = targetDate.daysInMonth() - prevDate.getDay() + i;
                    date.innerHTML = targetDate.daysInMonth() - prevDate.getDay() + i;

                    this.body.appendChild(date);                
                }
                ...
                for (let i = 1; i <= targetDate.daysInMonth(); i++) {
                    ...
                    status = this.checkDateStatus(i, month + 1, year) ? 'calendar__day_ordered' : 'calendar__day_available';

                    date.classList.add(status);
                    date.dataset.date = `${i}/${month + 1}/${year}`;
                    date.innerHTML = i;

                    date.addEventListener('click', () => date.classList.toggle('active'));

                    this.body.appendChild(date);
                }

                for (let i = 1; i <= (this.dimentions.height * this.dimentions.width) - targetDate.daysInMonth() - targetDate.getDay() + 1; i++) {
                    const date = document.createElement('button');
                    date.classList.add('button', 'calendar__day', 'js-calendar-day');
                    let status = 'calendar__day_disabled';
            ...
            checkDateStatus(day, month, year){
                return this.dates.filter(date => date.day === day && date.month === month && date.year === year).length;
            }
        }

        const faqContainerClick = () => {
            const target = event.target;

            if(target.classList.contains('js-question-trigger')){
                target.classList.toggle('active');
                target.nextElementSibling.classList.toggle('active');
                setHeightToFitContent(target.nextElementSibling, target.nextElementSibling.classList.contains('active'));
            }
        };
        ...
        const scrollTo = (selector, offset = -100) => {
            try{
                window.scrollTo({
                    top: window.pageYOffset + document.querySelector(selector).getBoundingClientRect().top + offset,
                    behavior: 'smooth'
                });
            }
            catch(e){
        ...
        const mediaPopupTriggerClick = () => {
            const target = event.target;
            const type = target.dataset.mediaType;
            const link = target.dataset.mediaLink;
            const popup = document.querySelector('.js-popup-media');
            const popupBody = popup.querySelector('.popup__body');

            popupBody.childNodes.forEach(node => {
                if(node.tagName && !node.classList.contains('js-popup-close')){
                    node.remove();
                }
            });
            ...
            switch(type){
                case 'picture':
                    ...
                    elem.classList.add('popup__media');
                    elem.setAttribute('src', link);
                    elem.setAttribute('alt', 'Фото в попапе');
                break;
                case 'video':
            ...
            popupBody.appendChild(elem);
            
            openPopup(popup);
        };

        if(isExists(heroSwiper)){

            const checkSliderPos = swiper => {
                swiper.el.querySelector('.js-swiper-next').classList.remove('hero__control_hidden');
                swiper.el.querySelector('.js-swiper-prev').classList.add('hero__control_hidden');

                if(swiper.isEnd){
                    swiper.el.querySelector('.js-swiper-next').classList.add('hero__control_hidden');
                    swiper.el.querySelector('.js-swiper-prev').classList.remove('hero__control_hidden');
                }
            };
        ...
        if(isExists(placesSwiper)){

            new Swiper(placesSwiper, {
                slidesPerView: 1,
                spaceBetween: 20,
                speed: 600,
                navigation: {
                    nextEl: '.places .js-swiper-next',
                    prevEl: '.places .js-swiper-prev',
                },
            });
        }

        if(isExists(hotelSwiper)){
        ...
        if(isExists(faqContainer)){
            faqContainer.addEventListener('click', faqContainerClick);
        }

        if(isExists(dropdownItems)){
            dropdownSelectInit(dropdownItems);
        }

        if(isExists(quizNode)){
            new Quiz(quizNode);
        }

        if(isExists(calendarNode)){
            new Calendar(calendarNode);
        }

        if(isExists(popupOpenTriggers)){
            popupOpenTriggers.forEach(trigger => trigger.addEventListener('click', openPopupByTrigger));
        }

        if(isExists(scrollLinks)){
        ...
        if(isExists(tabs)){
            tabs.forEach(container => container.addEventListener('click', () => {
                if(event.target.classList.contains('js-tab-trigger')){
                    const targetId = event.target.dataset.targetId;

                    showTab(container, targetId);
                }
            }));
        }

        if(isExists(popupSwipers)){
            popupSwipers.forEach(swiper => {
                new Swiper(swiper, {
                    slidesPerView: 1,
                    speed: 600,
                    spaceBetween: 15,
        ...
        if(isExists(gallery)){
            gallery.addEventListener('click', popupSliderTriggerClick('.js-popup-gallery-slider'));
        }

        if(isExists(placeItems)){

            const createEl = (elName, classList) => {
            ...
            placeItems.forEach((item, index) => {
                const sliderTriggers = item.querySelectorAll('.js-example');

                const popup = createEl('div', ['popup', 'js-popup', 'js-popup-close', `js-popup-place-${index}`]);

                const popupBody = createEl('div', ['popup__body', 'popup__body_slider', 'js-swiper-container']);
                const popupSwiperWrapper = createEl('div', ['swiper-wrapper']);

                const closeButton = createEl('button', ['button', 'popup__close', 'popup__close_media', 'js-popup-close']);

                sliderTriggers.forEach(trigger => {
                    const slide = createEl('div', ['swiper-slide', 'popup__slide']);

                    const slideImg = createEl('img', ['popup__img', 'swiper-lazy']);
                    slideImg.setAttribute('data-src', trigger.dataset.fullwidthImg);
                    slideImg.setAttribute('src', 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7');

                    const slidePreload = createEl('div', ['swiper-lazy-preloader']);

                    slide.appendChild(slideImg);
                    slide.appendChild(slidePreload);

                    popupSwiperWrapper.appendChild(slide);

                    trigger.addEventListener('click', () => popupBody.swiper.slideTo(Number(trigger.dataset.slideIndex) || 0, 0, true));
                });

                popupBody.appendChild(closeButton);
                popupBody.appendChild(popupSwiperWrapper);
                popup.appendChild(popupBody);
                document.body.appendChild(popup);

                new Swiper(popupBody, {
                    watchSlidesVisibility: true,
                    observer: true,
                    slidesPerView: 1,
                    speed: 600,
            ...
            popupCloseTriggers = document.querySelectorAll('.js-popup-close');
        }

        if(isExists(popupCloseTriggers)){
        ...
        mediaPopupTrigger.forEach(trigger => trigger.addEventListener('click', mediaPopupTriggerClick));
