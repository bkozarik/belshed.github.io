...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const mainSwiperNode = document.querySelector('.js-main-swiper');
        const mainSwiperPagination = document.querySelector('.js-pagination');
        const mainSwiperControls = document.querySelectorAll('.js-main-control');
        let mainSwiperPaginationSlide;
        const maintTitles = document.querySelectorAll('.js-main-title-item');
        const reviewsSwiperNode = document.querySelector('.js-reviews-swiper');
        ...
        const bodyBgMap = document.querySelector('.js-body-map');
        const scrollLinks = document.querySelectorAll('.js-scroll-link');
        const popupMarks = document.querySelectorAll('.js-popup-mark');
        const popupCities = document.querySelectorAll('.js-popup-city');
        const telInputs = document.querySelectorAll('input[type="tel"]');
        const formTextarea = document.querySelectorAll('.js-limited-textarea');
        const phoneButton = document.querySelector('.js-button-phone');
        ...
        const updateSlidePaginationItem = function(pagination) {
            const activePaginationItem = pagination.querySelector('.pagination__item_active');
            const activePaginationItemDimentions = {
                x: activePaginationItem.offsetLeft,
                y: activePaginationItem.offsetTop,
            };
            const paginationSlide = pagination.querySelector('.js-pagination-item-slide');
            paginationSlide.style.top = activePaginationItemDimentions.y + 'px';
            paginationSlide.style.left = activePaginationItemDimentions.x + 'px';
        };
        const updatePagination = function(swiper) {
            const paginationItems = mainSwiperPagination.querySelectorAll('.js-pagination-item');
            try {
                paginationItems.forEach(function(item) {
                    item.classList.remove('pagination__item_active');
                });
            } catch (e) {}
            const paginationActiveItem = paginationItems[swiper.activeIndex];
            try {
                paginationActiveItem.classList.add('pagination__item_active');
            } catch (e) {}
            try {
                updateSlidePaginationItem(mainSwiperPagination);
            } catch (e) {}
        };
        const showActualTitle = function(swiper) {
            maintTitles.forEach(function(item) {
                item.classList.remove('active');
            });
            maintTitles[swiper.activeIndex].classList.add('active');
        };
        const moveMainBg = function(swiper) {
            const bgHeight = bodyBg.getBoundingClientRect().height;
            const bodyHeightt = document.body.getBoundingClientRect().height;
            const heightDiff = bgHeight - bodyHeightt;
            const moveStep = heightDiff / swiper.slides.length;
            if (heightDiff > 0) {
                bodyBg.style.transform = "translateY(" + -1 * moveStep * swiper.activeIndex + "px)";
                bodyBgMap.style.transform = "translate(-50%, calc(" + -5 * moveStep * swiper.activeIndex + "px - 50%))";
            }
        };
        const mainSwiperInit = function() {
            const mainSwiper = new Swiper(mainSwiperNode, {
                slidesPerView: 1,
                direction: 'vertical',
                speed: 600,
                mousewheel: true,
                ...
                on: {
                    init: function() {
                        this.slides.forEach(function(slide, index) {
                            ...
                            paginationItem.classList.add('pagination__item', 'js-pagination-item');
                            paginationItem.setAttribute('tabindex', 0);
                            paginationItem.setAttribute('data-index', index);
                            paginationItem.appendChild(document.createElement('span'));
                            mainSwiperPagination.appendChild(paginationItem);
                        });
                        const paginationSlideItem = document.createElement('li');
                        paginationSlideItem.classList.add('pagination__item-slide', 'js-pagination-item-slide');
                        mainSwiperPagination.appendChild(paginationSlideItem);
                        updatePagination(this);
                        showActualTitle(this);
                        moveMainBg(this);
                        if (this.isEnd) {
                            document.querySelector('.main__control_next').classList.add('main__control_hidden');
                            document.querySelector('.main__control_back').classList.remove('main__control_hidden');
                        } else {
                            document.querySelector('.main__control_next').classList.remove('main__control_hidden');
                            document.querySelector('.main__control_back').classList.add('main__control_hidden');
                        }
                    },
                    slideChange: function() {
                        ...
                        if (!this.isBeginning && !this.isEnd) {
                            let defaultBgOpacity = 0.6;
                            bodyBg.style.opacity = defaultBgOpacity;
                            if (this.activeIndex == 3) {
                                bodyBg.style.opacity = '0.3';
                            }

                            this.$el[0].style.zIndex = 1;
                            
                            if (this.activeIndex == 4) {
                                this.$el[0].style.zIndex = 2;
                            }
                            ...
                            document.querySelector('.main__control_next').classList.add('main__control_hidden');
                            document.querySelector('.main__control_back').classList.remove('main__control_hidden');
                            document.querySelectorAll('.js-map iframe').forEach(function(map) {
                        ...
                        if (!this.isEnd) {
                            document.querySelector('.main__control_next').classList.remove('main__control_hidden');
                            document.querySelector('.main__control_back').classList.add('main__control_hidden');
                            document.querySelector('.valutes').classList.remove('valutes_visible');
                        }
                        if (this.isEnd) {
                            document.querySelector('.valutes').classList.add('valutes_visible');
                        }
                    },
                },
                breakpoints: {
            ...
            const paginationItems = mainSwiperPagination.querySelectorAll('.js-pagination-item');
            mainSwiperPaginationSlide = document.querySelector('.js-pagination-item-slide');
            paginationItems.forEach(function(item) {
                item.addEventListener('click', function() {
                    const slideIndex = item.dataset.index;
                    mainSwiper.slideTo(slideIndex, mainSwiper.params.speed, true);
                });
            });
            mainSwiperControls.forEach(function(control) {
                control.addEventListener('click', function() {
                    if (this.classList.contains('js-main-control-next')) {
                        mainSwiper.slideNext();
                    } else if (this.classList.contains('js-main-control-first')) {
                        mainSwiper.slideTo(0, mainSwiper.params.speed, true);
                    }
                });
            });
            scrollLinks.forEach(function(link) {
                link.addEventListener('click', function() {
                    ...
                    mainSwiper.slideTo(event.target.getAttribute('href'), mainSwiper.params.speed, true);
        ...
        const reviewsSwiperInit = function() {
            const reviewsSwiper = new Swiper(reviewsSwiperNode, {
                slidesPerView: 1,
                navigation: {
        ...
        const dropdownSelectInit = function(dropdownItems) {
            const createOptionPopupItem = function(option, index) {
            ...
            const setTriggerText = function(item, data) {
                ...
                dropdownTrigger.addEventListener('click', function() {
                    ...
                    document.addEventListener('click', function() {
                        ...
                        if (!target.parentNode.classList.contains('js-dropdown-trigger') && !target.classList.contains('js-dropdown-trigger')) {
                            document.querySelectorAll('.dropdown__list').forEach(function(item) {
                                item.classList.remove('active');
                            });
                            document.querySelectorAll('.js-dropdown-trigger').forEach(function(item) {
                                item.classList.remove('active');
                            });
                        }
                    });
        ...
        const openPopup = function(popup) {
            closePopups();
            const targetPopup = typeof(popup) == 'string' ? document.querySelector(popup) : popup instanceof Element ? popup : null;
            if (targetPopup.classList.contains('js-popup-answer')) {
                document.querySelectorAll('.js-popup-answer').forEach(function(popup) {
        ...
        const openPopupByTrigger = function() {
            let target = event.target;
        ...
        const resizeHandler = function() {
            moveMainBg(mainSwiperNode.swiper);
            ...
            if (window.innerWidth <= 920) {
                ...
                document.body.appendChild(document.querySelector('.faq__answers-list'));
                document.querySelectorAll('.js-popup-answer').forEach(function(popup) {
                    popup.classList.add('js-popup');
                });
                // document.querySelector('.hero__body').appendChild(document.querySelector('.contacts__copyright'));
            } else {
                // document.querySelector('.contacts').appendChild(document.querySelector('.contacts__copyright'));
                document.querySelector('.faq__wrap').appendChild(document.querySelector('.faq__answers-list'));
                document.querySelectorAll('.js-popup-answer').forEach(function(popup, index) {
                    if (index == 0) {
        ...
        const mapMarksInit = function() {
            popupMarks.forEach(function(mark) {
                mark.addEventListener('mouseover', function() {
                    const targetMarks = Array.from(popupMarks).filter(function(searchingMark) {
                        return searchingMark.dataset.index == mark.dataset.index;
                    });
                    targetMarks.forEach(function(mark) {
                ...
                mark.addEventListener('click', function() {
                    ...
                    popupCities.forEach(function(city) {
                    ...
                    targetMarks.forEach(function(mark) {
                        ...
                        const targetCities = Array.from(popupCities).filter(function(searchingCity) {
                            return searchingCity.dataset.index == mark.dataset.index;
                        });
            ...
            popupCities.forEach(function(city) {
                city.addEventListener('click', function() {
                    ...
                    const targetMarks = Array.from(popupMarks).filter(function(searchingMark) {
                        return searchingMark.dataset.index == city.dataset.target;
                    });
        ...
        const limitInput = function() {
            ...
            textarea.style.height = parseInt(getComputedStyle(textarea).height) + textarea.scrollTop + 'px';
        };
        const toggleMenu = function(state) {
            state = typeof state !== 'null' ? state : 1;
            if (state == null) {
                return function() {
        ...
        const updateValutes = function() {
            const valutes = ['USD', 'EUR'];
            const valutesList = document.querySelector('.js-valutes');
            const apiUrl = 'https://www.cbr-xml-daily.ru/daily_json.js';
            fetch(apiUrl).then(function(response) {
                return response.json()
            }).then(function(response) {
                while (valutesList.lastChild) {
                    valutesList.lastChild.remove();
                }
                const fetchedValutesList = response.Valute;
                valutes.forEach(function(val) {
                    const node = document.createElement('li');
                    node.classList.add('valutes__item');
                    node.innerHTML = fetchedValutesList[val].CharCode + ' - ' + Math.round(fetchedValutesList[val].Value * 100) / 100 + '₽';
                    valutesList.appendChild(node);
                });
                setTimeout(updateValutes, 20000);
            });
        };
        if (isExists(popupTriggers)) {
            popupTriggers.forEach(function(trigger) {
                event.stopPropagation();
                trigger.addEventListener('click', openPopupByTrigger);
            });
        }
        ...
        if (isExists(formTextarea)) {
            formTextarea.forEach(function(textarea) {
                textarea.addEventListener('change', limitInput);
                textarea.addEventListener('input', limitInput);
                textarea.addEventListener('blur', limitInput);
            });
        }
        if (isExists(phoneButton)) {
            phoneButton.addEventListener('click', function(event) {
                if (window.innerWidth >= 920) {
                    event.stopPropagation();
                    event.preventDefault();
                    phoneButton.nextElementSibling.classList.toggle('active');
                    document.addEventListener('click', function(event) {
                        if (!event.target.classList.contains('button-list__popup')) {
                            phoneButton.nextElementSibling.classList.remove('active');
                        }
                    });
                }
            });
        ...
        document.querySelectorAll('.js-popup-answer').forEach(function(answer) {
            const closeBtn = answer.querySelector('.js-popup-close');
